...
bffp_frontYDimension = 392;
bffp_frontZDimension = 19.2;

bffp_rearXDimension = bffp_frontXDimension;
bffp_rearYDimension = 150;
bffp_rearZDimension = bffp_frontZDimension;
...
bffp_middleConnectorCutoutYDimension = 21.5;
bffp_middleConnectorCutoutZDimension = 8.2;
            ...
            }
            {
                union()
                {
                    ...
                    translate( [bffp_topOverhang, -1 * (bffp_frontYDimension) + profileSize + 7, -1 * profileSize + bffp_topOverhang + 0.9] )
                    {
                        ...
                        translate( [-1 * bffp_topOverhang, -1 * bffp_topOverhang, bffp_topOverhang + 6.3] )
                        {
...
module topPlateCutouts( isFront = 1 )
{
    union()
    {
        ...
        translate( [xAxisProfileLength - (profileSize * 2) + bffp_topOverhang - 105, yAxisProfileLength + bffp_topOverhang - bffp_rearCutoutYDimension - profileSize, bffp_topOverhang - 6.8] )
        {
        ...
        translate( [ -1, yAxisProfileLength - zAxisTowerDistanceFromEnd - 129.2, -5.9] )
        {
            translate( [0, 40, 0] ) cube( [bf_wallThickness + 31, pieceDepth + 100, profileSize + 6] );

            translate( [ xAxisProfileLength + profileSize, 40, 0] )
            {
                cube( [bf_wallThickness + 31, pieceDepth + 100, profileSize + 6] );
            }

            translate( [-5, 40, 0] )
            {
                cube( [bf_wallThickness + 31, pieceDepth + 82.2, profileSize + 6] );

                translate( [ xAxisProfileLength + profileSize + 10, 0, 0] )
                {
                    cube( [bf_wallThickness + 31, pieceDepth + 82.2, profileSize + 6] );
                }
            }
        ...
        translate( [-1, yAxisProfileLength + bffp_topOverhang - 1, -1 * bffp_topOverhang + 0.2] )
        {
        ...
        translate( [21, bffp_frontYDimension - 152, 11.1 - bffp_middleConnectorCutoutZDimension - 3.1 - 19.8 + 12] )
        {
...
module topFrameMountThroughHoles( isFront = 1 )
{
    translate( [bffp_topOverhang + (profileSize/2), bffp_topOverhang + (profileSize/2), -10] )
    {
        ...
        // translate( [(bffp_frontYDimension/2) + (bffp_middleCutoutXDimension/2) - 125, bffp_frontYDimension - profileSize - 6, 0] )
        // {
        //     m5ThroughHole_duplicate( height = 40 );
        //     translate( [0, 0, 5.2 + 4.7] )
        //     {
            ...
            union()
            {
                ...
                translate([xAxisProfileLength + (profileSize * 2) - bfpi_actualXDimension + 3, bfpi_platformFrontOffset, profileSize] )
                {
                    translate( [0, 0, bfpi_platformBottomBuffer] )
                    {
                        ...
                        {
                            translate( [0.5, -38, bfpi_externalMountBlockHeight] )
							{
								cube( [bfpi_usbReliefThickness, bfpi_usbReliefWidth, bfpi_usbReliefHeight] );

								translate( [bfpi_usbReliefThickness - 1, bfpi_usbReliefWidth - 1, 0] )
								{
									hull()
									{
										{
										...
										}
										{
											...
											{
												cube( [1, 1, 1] );
												translate( [0, 0, bfpi_usbReliefHeight - 1] ) cube( [1, 1, 1] );

												translate( [-1 * bfpi_usbReliefThickness + 1, 0, bfpi_usbReliefHeight - 1] ) cube( [1, 1, 1] );
												translate( [-1 * bfpi_usbReliefThickness + 1, 0, 0] ) cube( [1, 1, 1] );

												translate( [-1 * bfpi_usbReliefThickness + 0.1, 17, bfpi_usbReliefHeight - 0.1] ) cube( [0.1, 0.1, 0.1] );
												translate( [-1 * bfpi_usbReliefThickness + 0.1, 17, 0] ) cube( [0.1, 0.1, 0.1] );
											}
										}
									}
                ...
                translate([xAxisProfileLength + (profileSize * 2) - bfpi_actualXDimension + 3, bfpi_platformFrontOffset + 5, profileSize] )
                {
                    ...
                    {
                        translate( [-1 * (bfpi_fanMainCutoutDiameter/2) + 29.2, 25.75 - 0.5, 25 - 0.5] )
                        {
                            cube( [bfpi_externalMountBlockDepth + 0.5, bfpi_externalMountBlockWidth + 1, bfpi_externalMountBlockHeight + 1] );

                            translate( [-10, 0.5 + (bfpi_externalMountBlockWidth/2), 0.5 + (bfpi_externalMountBlockHeight/2)] )
                            {
                                rotate( [0, 90, 0] )
                                {
...
module rightPanelFrameMountThroughHoles()
{
    translate( [xAxisProfileLength + profileSize + 7, (profileSize/2), (profileSize / 2)] )
    {
        rotate( [0, 90, 0] )
        {
            m5ThroughHole_duplicate( height = 20 );
...
bfpi_usbReliefThickness = bffp_thickness - 2.7;
...
bfpi_fanDistanceBetweenBolts = 23.8;
bfpi_fanPlatformXYDimension = 31;
...
module piFanMountThroughHoles()
{
    union()
    {
        translate( [((bfpi_fanPlatformXYDimension - bfpi_fanDistanceBetweenBolts)/2), ((bfpi_fanPlatformXYDimension - bfpi_fanDistanceBetweenBolts)/2), -1] )
        {
            m3ThroughHole();
            translate( [bfpi_fanDistanceBetweenBolts, 0, 0] ) m3ThroughHole();
            translate( [0, bfpi_fanDistanceBetweenBolts, 0] ) m3ThroughHole();
            translate( [bfpi_fanDistanceBetweenBolts, bfpi_fanDistanceBetweenBolts, 0] ) m3ThroughHole();
        }

        translate( [(bfpi_fanPlatformXYDimension/2), (bfpi_fanPlatformXYDimension/2), -1] )
        {
            cylinder( d = bfpi_fanMainCutoutDiameter, h = bfpi_fanPlatformZDimension + 2 );
        }

        translate( [40, (bfpi_fanPlatformXYDimension/2), (bfpi_externalMountBlockHeight/2)] )
        {
            rotate( [0, 90, 0] )
            {
        ...
        {
            {
                translate( [0, 0, bfpi_platformBottomBuffer] )
                {
                    ...
                    // Fan mount in-place
                    // translate( [-1 * (bfpi_fanMainCutoutDiameter/2) + 17, 25.75, 23] )
                    // {
                    //     piFanMount();
                    // }
                }
